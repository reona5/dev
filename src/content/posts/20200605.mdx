...
など、気になる点もいくつかありました。
Vue や React は去年それぞれ 1 ヶ月ほど触った経験があったので、TypeScript と相性が良いと言われている React を選択しました。
...
基本的に、`now` （もしくは `vercel` ）コマンドでデプロイできますが、GitHub リポジトリと紐付ければ master ブランチへの Push 時にデプロイしてくれて便利です。
